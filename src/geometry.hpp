...
struct Point3D
{
    ...
    Point3D& operator*=(const float scalar)
    {
        ...
        // y() *= scalar;
        // z() *= scalar;
        std::transform(values.begin(), values.end(), values.begin(),[scalar](float tmp){return tmp*=scalar;});
    ...
    float length() const { return std::sqrt(std::reduce(values.begin(), values.end(),0.0,[](float f1,float f2){return f1+f2*f2; })); }
